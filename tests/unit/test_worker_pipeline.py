...
def test_worker_counts_repeated_auth_log_lines(
    tmp_path: pytest.TempPathFactory,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db_path = tmp_path / "sentinel.db"
    ...
    assert len(alerts) == 1
    assert alerts[0].ip == "10.241.1.119"
    assert alerts[0].attempts == 5
    assert alerts[0].blocked is True


def test_worker_updates_existing_alert_for_repeat_breaches(
    tmp_path: pytest.TempPathFactory,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db_path = tmp_path / "sentinel.db"
    ...
    assert len(alerts) == 1
    assert alerts[0].ip == "203.0.113.10"
    assert alerts[0].attempts == 4
    assert alerts[0].blocked is True
